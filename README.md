# Proyecto_Katas_Python
Proyecto en el que se realizan las katas propuesta.
# Descripción
En este proyecto se realizan las katas propuestas.
Para ello se utiliza la extensión de Jupyter Notebook en Visual Estudio Code. 
En el se aplican los conocimientos aprendidos, entre ellos:
- Manejo de tipos de datos básicos y funciones incorporadas.
- Manejo de estructuras de datos en Python y sus métodos.
- Manejo de condicionales.
- Manejo de estructuras de iteración.
- Manejo de funciones en Python.
- Manejo de Clases y entendimiento de la programación orientada a objetos.
- Uso de módulos y librerías vistos en el temario.
- Buenas prácticas.
# Archivos adjuntos:
- Resolución_Katas.ipynb
# Autor:
- Antonio Fernández
- @Antonio28863

